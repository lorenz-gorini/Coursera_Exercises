...
% initialize deltas
delta_2_extra=zeros(m,hidden_layer_size+1);
...
for i=1:m
  % Forward Propagation
  z_2(i,:)=(Theta1*X(i,:)')'; % (25x(400+1)*(400+1))'= vector(25)
  A_2(i,:)=[1 sigmoid(z_2(i,:))];  % vector(25+1)
  z_3(i,:)=(Theta2*A_2(i,:)')';	% ((25+1)x10*(25+1))'= vector(10)
  A_3(i,:)=sigmoid(z_3(i,:)); % vector(10)=result

  % Back Propagation
  delta_3=A_3(i,:)-y_matrix(i,:); # vector 1x10
  % Eliminating the term related to the bias value 1 !!! (delta-variation not possible on that)
  delta_2_extra=(delta_3*Theta2).*A_2(i,:).*(1-A_2(i,:)); # delta_3*Theta_2 = 1x10 * 10x(25+1) = vector 1x26
  delta_2=delta_2_extra(2:end);
  Delta_major_1=Delta_major_1+delta_2'*X(i,:); #matrix 25x401 skipping z2(0)=(25*1)
  Delta_major_2=Delta_major_2+delta_3'*A_2(i,:); #matrix 10x26
endfor
